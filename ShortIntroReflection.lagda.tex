...
We \emph{also} define equality, which is an equivalence relation and a congruence, as well as some more associated functions.
...
If we have many such rewritings, then it would be too much trouble to do it manually. Another way is to translate it to, say, a string, and then write a program to replace "\AgdaString{n + 0}" with "\AgdaString{n}". We could then translate the string back to terms and somehow insert
"\AgdaString{lemma-n+0}" at the right place. We would probably need some structure that is better than strings to support recording places where "\AgdaString{lemma-n+0}" is needed, though.
...
The last is an expression with all ``\AgdaString{n + zero}'' replace by ``n''. Note that we can prove (2) by induction on the AST.
...
A parameterised \AgdaPostulate{TC} computation that try to unify hole with provide solution. We only use agda buitins, so it's a little mess. Using do-notation will be cleaner. But still easy to see what's there: normalize the type of the hole >>= take out \AgdaFunction{lhs} >>= using \AgdaFunction{q'⟦⟧} on lhs to generate a proof sol >>= unify hole and sol.
