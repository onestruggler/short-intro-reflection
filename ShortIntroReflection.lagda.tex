...
\author{Xiaoning Bian and Andrew Christopher Hawk}
...
\begin{enumerate}
  \item The kind that is supported by Agda's builtin keywords (``\AgdaKeyword{quote}'', ``\AgdaKeyword{unquote}'', \ldots{}), which reflects on an Agda term by manipulating its Agda-AST representation. No parsing is needed here, as we can just quote the terms to get the Agda-AST representation.\label{keywordReflectionItem}
  \item The kind that doesn't need keyword support, which reflects on an Agda term by manipulating a user-defined AST that is somehow "isomorphic" to the term's structure, i.e., captures the main structure of the term. As an example, the structure of the term \AgdaBound{x} \AgdaOperator{\AgdaFunction{+}} \AgdaBound{y} \AgdaOperator{\AgdaFunction{*}} \AgdaBound{z}" with \AgdaBound{x} \AgdaBound{y} \AgdaBound{z} being variables, can be captured by a syntax tree. The term-to-AST parsing is manual, i.e., we have to manually type ``\AgdaBound{x} \AgdaOperator{\AgdaInductiveConstructor{:+}} \AgdaBound{y} \AgdaOperator{\AgdaInductiveConstructor{:*}} \AgdaBound{z}'' to build the AST for the term.\label{astTypeReflectionItem}
\end{enumerate}

Both share ``reflection'', i.e., both manipulate the term (program) by AST and do not directly execute the term.

Both \ref{keywordReflectionItem} and \ref{astTypeReflectionItem} are used in Agda programming. Combing both seems good, and, actually, many have done so --- a quick GitHub search for ``agda reflection'' should yield a couple of interesting repos.

``Combining both'' actually refers to the following sequence of steps:

\begin{enumerate}
  \item Use \ref{keywordReflectionItem} to get Agda-AST automatically.
  \item Translate the resulting Agda-AST representation to a user-defined AST.
  \item Finally, do the proof under \ref{astTypeReflectionItem}.
\end{enumerate}

This process prevents \ref{astTypeReflectionItem}'s manual typing part. We will, however, see that there are complications in this combining.
...
The last is an expression with all ``\AgdaString{n + zero}'' replaced by ``\AgdaString{n}''. Note that we can prove (2) by induction on the AST.
